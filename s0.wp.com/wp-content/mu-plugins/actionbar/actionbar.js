(function() {
    ...
    if (notifyPostsToggle) {
        notifyPostsToggle.addEventListener('click', (e) => {
            ...
            const restPath = `/read/sites/${fbd.siteID}/notification-subscriptions/${isEnabling?'new':'delete'}`;
    ...
    if (emailPostsToggle) {
        emailPostsToggle.addEventListener('click', (e) => {
            e.preventDefault();
            const isEnabling = e.target.parentElement.classList.toggle('is-checked');
            const restPath = `/read/site/${fbd.siteID}/post_email_subscriptions/${isEnabling?'new':'delete'}`;
            let body = {};
            if (isEnabling) {
                body.delivery_frequency = fbd.subsEmailDefault;
                frequencyOptions.forEach((opt) => opt.parentElement.classList.remove('is-selected'));
                actionbar.querySelector(`.frequency-${fbd.subsEmailDefault}`).parentElement.classList.add('is-selected');
            }
            actionbar.querySelector('#email-new-posts-details').classList.toggle('is-visible', isEnabling);
    ...
    if (emailCommentsToggle) {
        emailCommentsToggle.addEventListener('click', (e) => {
            e.preventDefault();
            const isEnabling = e.target.parentElement.classList.toggle('is-checked');
            const restPath = `/read/site/${fbd.siteID}/comment_email_subscriptions/${isEnabling?'new':'delete'}`;
    ...
    const onFrequencyOptionClick = (e, frequencyKey) => {
        ...
        postToApi(restPath, 'rest/v1.2', {
            delivery_frequency: frequencyKey
        });
    };
    ...
    statsOnClick('.actnbr-customize a', 'customized');
    statsOnClick('.actnbr-folded-customize a', 'customized');
    ...
    if (shortlink) {
        shortlink.addEventListener('click', bumpStat('copied_shortlink'));
    }
    const bubbleForm = actionbar.querySelector('.actnbr-follow-bubble form');
    if (bubbleForm) {
        bubbleForm.addEventListener('submit', createStatsBumperEventHandler('submit_follow_form', () => {
    ...
    function handleScroll() {
        ...
        if (scrollTop < lastScrollTop) {
            ...
            if (!isHidden && document.querySelectorAll('#actionbar > ul > li:not(.actnbr-hidden) > .actnbr-popover').length === 0) {
    ...
    function showActionBarStatusMessage(message) {
        follow && follow.classList.add('no-display');
        unfollow && unfollow.classList.remove('no-display');
